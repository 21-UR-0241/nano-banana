...
import { Sparkles, ImagePlus, RefreshCw, Heart, Mail, Shield, FileText, Zap, Github, LogOut, User } from "lucide-react";
...
import { collection, addDoc, query, orderBy, onSnapshot, doc, updateDoc, deleteDoc, where } from "firebase/firestore";
...
  }, [user]);
  ...
  // 🎨 Add Prompt Variation Function
  const addPromptVariation = useCallback((basePrompt: string): string => {
    // Lighting variations
    ...
    // Style variations for product photography
    ...
    const lighting = lightingVariations[Math.floor(Math.random() * lightingVariations.length)];
    ...
    const atmosphere = atmosphereVariations[Math.floor(Math.random() * atmosphereVariations.length)];
    const background = backgroundVariations[Math.floor(Math.random() * backgroundVariations.length)];
    const style = styleVariations[Math.floor(Math.random() * styleVariations.length)];
    const composition = compositionVariations[Math.floor(Math.random() * compositionVariations.length)];

    // Randomly decide which modifiers to include (3-5 of them)
    const allModifiers = [lighting, quality, camera, atmosphere, background, style, composition];
    const numModifiers = 3 + Math.floor(Math.random() * 3); // 3-5 modifiers
    const shuffled = allModifiers.sort(() => Math.random() - 0.5);
    const selectedModifiers = shuffled.slice(0, numModifiers);

    // Add temperature variation (subtle changes)
    const temperatureWords = ['', 'slightly ', 'subtly ', 'gently ', 'very '];
    const temperature = temperatureWords[Math.floor(Math.random() * temperatureWords.length)];

    // Add a unique variation identifier
    const uniqueId = `var${Math.floor(Math.random() * 100000)}`;
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 via-blue-50 via-purple-50 to-indigo-50 dark:from-slate-950 dark:via-slate-900 dark:via-purple-950 dark:to-indigo-950">
  ...
  if (onboardingComplete) {
    const promptData = buildPrompt(onboardingData as OnboardingData);
    const defaultPrompt = Object.keys(onboardingData).length > 0 ? formatPromptForAI(promptData) : "Create a professional marketing image for a modern business";
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 via-purple-50 to-indigo-50 dark:from-slate-950 dark:via-slate-900 dark:via-purple-950 dark:to-indigo-950 flex flex-col relative overflow-hidden">
          ...
          <div className="container mx-auto px-6 py-6 max-w-7xl relative">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-3">
                  <Avatar className="w-8 h-8 border-2 border-white/20 dark:border-slate-700/50 shadow-lg">
                  ...
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    className="gap-2 px-4 py-2 text-sm border-2 border-slate-300 dark:border-slate-700 hover:bg-gradient-to-r hover:from-red-50 hover:to-pink-50 dark:hover:from-red-950/30 dark:hover:to-pink-950/30 hover:border-red-300 dark:hover:border-red-700 hover:text-red-600 dark:hover:text-red-400 transition-all duration-300 shadow-lg hover:shadow-xl hover:shadow-red-500/20 backdrop-blur-sm"
                  >
        ...
        {/* Privacy, Terms, and Contact Dialogs remain the same... */}
        {/* (I'll skip these for brevity since they're unchanged) */}
        ...
        {/* Terms and Contact Dialogs remain the same... */}
